...
public class ProcessoLocacao
{
    ...
    private object serviceBusSender;
        ...
        ServiceBusMessageActions messageActions)
    {
        ...
        try
        {
            ...
            var command = new SqlCommand(
                "INSERT INTO Locacao (Nome, Email, Modelo, Ano, TempoAluguel, Data) VALUES (@Nome, @Email, @Modelo, @Ano, @TempoAluguel, @Data)",
                connection);
            ...
            var ServiceBusConnection = _configuration.GetConnectionString("ServiceBusConnection");
            var ServiceBusQueue = _configuration.GetValue<string>("ServiceBusQueue");
        ...
        {
            _logger.LogError("Erro ao processar a mensagem: {messageId}", message.MessageId);
            await messageActions.DeadLetterMessageAsync(message, 
                null,
    ...
    private void sendMessageToPay(string? serviceBusConnection, string? serviceBusQueue, RentModel rentModel)
    {
        ...
        message.ApplicationProperties.Add("Data", rentModel.data.ToString("o"));
